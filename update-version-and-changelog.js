...
async function updateVersion() {
	...
	if ( ! fs.existsSync( './exoole.php' ) ) {
		console.error( '❌ Error: exoole.php file not found.' );
		process.exit( 1 );
	}
	...
	// update exoole.php version
	const pluginPhpFile = fs.readFileSync( './exoole.php', 'utf8' );
	const newPluginPhpFile = pluginPhpFile.replace(
		/Version: (.*)/,
		`Version: ${ newVersion }`
	);
	fs.writeFileSync( './exoole.php', newPluginPhpFile );
	console.info(
		'✅  exoole.php file version updated',
		currentTag,
		'=>',
		newTag
	);
